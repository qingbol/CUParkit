<?php
    ...
    */
    ...
    $parkOn->fillAttributes($data);
    
    // Create Owner entry in the database using the data model's function
    if($parkOn->create()) {
        echo json_encode(array('message' => 'parkOn Successfully Created'));
    } else {
        echo json_encode(array('message' => 'ParkOn Not Created',
                                'PasswordMsg' => $parkOn->getMsg()
        ));
