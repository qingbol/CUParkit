<?php
    ...
    */
    ...
    $manageRecord->setAttr("mid", $data->mid);
    $manageRecord->setAttr("rcd_index", $data->rcd_index);
    ...
    if($manageRecord->delete()) {
        echo json_encode(array('message' => 'ManageRecord Successfully Deleted'));
        // echo json_encode(array('message' => 'Vehicle Successfully Deleted',
                                // 'id: ' => $_SESSION['id']));
        // TODO: If the delete request came from a normal user, log the normal user out
        //      because they just deleted their database entry.

    } else {
        echo json_encode(array('message' => 'ManageRecord Not Deleted'));
    }
